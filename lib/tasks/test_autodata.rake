namespace :import do
  ...
  task autodata: :environment do
    ...
    if ENV['BRAND'].present?
      ...
      # STDIN.gets es necesario porque gets normal falla dentro de rake tasks
    ...
    # Iniciamos vacío (nils) solo para usar el método find_brand_data
    finder = AutoDataScraper.new(nil, nil, nil) 
    ...
    if brand_data
      ...
      # Iniciamos el scraper con los datos OFICIALES
      scraper = AutoDataScraper.new(brand_data[:name], brand_data[:url], brand_data[:id])
      scraper.run
    else
  ...
  class AutoDataScraper
    ...
    def initialize(brand_name, start_url, brand_id)
      ...
      if @brand_id && @brand_name
        ...
        # Si el nombre en la DB era diferente lo actualizamos
    ...
    def find_brand_data(user_input)
      ...
      begin
        ...
        # Sacar el ID de la URL usando Regex
    ...
    def run
      ...
      # Detectar si estamos en GitHub (CI=true) o local
